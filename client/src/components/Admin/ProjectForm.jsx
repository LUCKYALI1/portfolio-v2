...
const ProjectForm = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!imageFile) {
      setStatusFeedback({
        type: 'error',
        text: '[CRITICAL]: Image asset file is required.',
      });
      return;
    }
    ...
    // FormData for multipart/form-data request
    ...
    uploadForm.append('techStack', cleanTechStack.join(','));
    ...
    try {
      ...
      const failMsg =
        ...
        error.message ||
        'Handshake failed during deployment.';

      setStatusFeedback({
        type: 'error',
        text: `[CRITICAL]: ${failMsg}`,
      });
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="relative z-10 w-full max-w-2xl bg-[#09090b]/90 border border-zinc-900 rounded-2xl p-6 md:p-10 shadow-2xl backdrop-blur-md space-y-8">
        ...
        <div className="space-y-2 text-center sm:text-left border-b border-zinc-900 pb-6">
          ...
          <p className="text-zinc-500 text-xs leading-relaxed max-w-md">
            Welcome, Lucky Ali. Deploy new interactive applications,
            architectures, and portfolio builds to the live system database.
          </p>
        </div>
            ...
            >
              ...
              {imageFile ? (
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-zinc-400 max-w-xs truncate mx-auto">
                    {imageFile.name}
                  </p>
                ...
                <div className="space-y-1 font-mono text-xs">
                  <p className="text-zinc-400">
                    Drag & drop project layout matrix here
                  </p>

                  <p className="text-zinc-600 text-[10px]">
                    or{' '}
                    <span className="text-cyan-400 hover:underline">
                      browse terminal file system
                    </span>
                  </p>
                </div>
              )}
